...
lazyLoading(int virAddr,int phyAddr,int size,bool isWrite){
    ...
    if (isWrite){
        while (pos<size){
            currentThread->space->virtualMemory[virAddr+pos] = machine->mainMemory[phyAddr+pos];
            pos++;
        }

    }else{
        while (pos<size){
            machine->mainMemory[phyAddr+pos] = currentThread->space->virtualMemory[virAddr+pos];
            pos++;
        }
    }
    return true;
